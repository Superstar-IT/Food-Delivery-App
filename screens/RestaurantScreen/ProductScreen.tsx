...
const ProductScreen = props => {
  ...
  useEffect(() => {
    if (!!options) {
      setAllGood(
        !selected
          .map(item => item.options.some(item => item?.selected === true))
          .includes(false)
      );
  ...
  return (
    <View style={{ flex: 1 }}>
      <ModalViewHOC
        ...
        image={
          image ||
          "https://previews.123rf.com/images/topform8/topform81307/topform8130700214/20674548-fast-food-seamless-background.jpg"
        }
      >
        {() => (
          <>
            <View style={{ padding: 30 }}>
              <Text style={{ fontFamily: "ProximaNova-Bold", fontSize: 24 }}>
                {name}
              </Text>
            ...
            <View style={{ flex: 1 }}>
              ...
              <TextInput
                ...
                multiline
                placeholder="Specify this item including any condiments, or item specifications"
                style={{
                  minHeight: 50,
                  paddingLeft: 20,
                  borderRadius: 6,
      ...
      {!!!amount && (
        <BottomOrderItem
          disabled={!allGood}
          text="Add to cart"
          price={Number(_price) + Number(optionsPrice)}
          handlePress={() => {
      ...
      {!!amount && (
        <BottomOrderItem
          ...
          price={Number(_price) + Number(optionsPrice)}
          handlePress={() => {
            // IF PRODUCT IN CART THEN UPDATE AMOUNT
            dispatch({
              type: "UPDATE_CART",
              payload: {
                ...props.route.params,
...
function OptionsList({ selected, setSelected, option }) {
  ...
  const getIndex = function (title, text) {
    ...
    const optionIndex = selected[titleIndex].options.findIndex(
      item => item.text === text
    ); // FIND WHICH INDEX TO update inside object "SELECT DRINK" E.Q: IF text === 'Coca-Coca', find index of it.
  ...
  return option.options.map(item => {
    ...
    function handleChangeSignle() {
      setSelected(
        produce(selected, draftState => {
          draftState[index1].options.map(item => {
          ...
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            <View
              style={{
                ...
                height: 20,
                backgroundColor:
                  selected[index1].options[index2]?.selected === true
                    ? "rgb(0,176,97)"
                    : "transparent",
