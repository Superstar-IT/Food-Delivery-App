...
const TabBar = ({ headerAnimation, scrollPos, scrollRef, scrollY }) => {
  ...
  useMemo(
    () =>
      hozRef.current?.scrollTo({
        x: tabsLayout.filter(item => item.name === current)[0]?.x - 20,
        y: 0,
        animated: true
      }),
    [current]
  ...
  function handleLayout(event, name) {
    if (tabsLayout.filter(item => item?.name === name).length === 0)
      setTabsLayout([...tabsLayout, { name, x: event.nativeEvent.layout.x }]);
  }
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() =>
        scrollRef.current
          .getNode()
          .scrollTo({ x: 0, y: item.y, animated: true })
      }
      onLayout={props => handleLayout(props, item.name)}
    >
      <View
        style={{
          ...
          style={{
            fontFamily:
              current === item.name
                ...
                : "ProximaNova-Bold",
            fontSize: current === item.name ? 18 : 12,
            color: current === item.name ? "black" : "grey",
            textAlign: "center"
          }}
        >
