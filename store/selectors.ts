...
//       (totalsum: any, data: any) =>
//         (totalsum +=
//           Number(data.price) * Number(data.amount) +
//           getOptionsPrice(data?.options, data.amount)),
//       0
//     );
//   }
// );
...
export const priceSelector = createSelector(
  ...
  (itemsByID: any) => {
    return Object.values(itemsByID).reduce(
      (totalsum: any, data: any) => (totalsum += Number(data.price)),
      0
    );
  }
);
export function getOptionsPrice(options: any, amount?: number) {
  debugger;
  //   return (
  //     options?.reduce((sum: number, val: any) => {
  //       sum +=
  //         val.options.reduce((sum2: number, val2: any) => {
...
export const upcomingSelector = createSelector(
  ...
  orders =>
    orders.filter((order: IOrder) =>
      moment(order.delivery.delivery_time).isAfter(Date.now(), "day")
    )
);
...
export const pastSelector = createSelector(
  ...
  orders =>
    orders.filter((order: IOrder) =>
      moment(order.delivery.delivery_time).isBefore(
...
export const todaySelector = createSelector(
  ...
  orders =>
    orders.filter((order: IOrder) =>
      moment(order.delivery.delivery_time).isSame(Date.now(), "day")
    ) || []
);
...
export const itemsAmountSelector = createSelector(
  ...
  items =>
    Object.values(items).reduce(
      (sum: number, value: any) => (sum += Number(value.amount)),
      0
    )
);

export const restaurantSelector = (id: number) =>
  createSelector(
